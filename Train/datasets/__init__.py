...
def create_dataset(opt) -> dict:
    ...
    ret = {'train': DataLoader(train_ds, batch_size=batch_size, num_workers=num_workers, shuffle=True, drop_last=True),
           'eval': DataLoader(eval_ds, batch_size=batch_size, num_workers=num_workers, shuffle=True, drop_last=False)}
    ...
    if 'test_dataset' in opt.keys(): # Use standalone test dataset
        ...
        ret['test']   = DataLoader(test_ds, batch_size=batch_size, num_workers=num_workers, shuffle=True, drop_last=False)
